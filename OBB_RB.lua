...
GetKey.MouseButton1Click:Connect(function()
   setclipboard("https://work.ink/1V4b/lx7xnjlw")
end)

local function validateKey(key)
    return key == "13a05820-bb18-40a3-901a-66b585c280b0" -- Replace this with your key
end

local function GetUserRole(userId)
    local ownerIds = {0, 0} -- List of owner user IDs
    local vipIds = {0, 0} -- List of vip user IDs
    
    if table.find(ownerIds, userId) then
        return "Owner"
    elseif table.find(vipIds, userId) then
        return "vip"
    else
        return "Member"
    end
...
elseif UserRole == "vip" then
    roleMessage = roleMessage .. "vip"
else
...
Notification.Notify(roleMessage, "Welcome To City Script", "rbxassetid://18251750733", {
    Duration = 5,       
    Main = {
        Rounding = true,
    }
...
CheckKey.MouseButton1Click:Connect(function()
    ...
    elseif UserRole == "Owner"
        TextBox.PlaceholderText = "HELLO OWNER"
        wait(2)
        TextBox.Text = ""
        wait(1)
        ScreenGui:Destroy()
        loadstring(game:HttpGet("https://raw.githubusercontent.com/ASSISTASD/SCURRY/main/21stjfhihfsagjb76%245w9.lua"))()
    elseif UserRole == "vip"
        TextBox.PlaceholderText = "HELLO VIP USER DON'T NEED KEY"
        wait(2)
        TextBox.Text = ""
        wait(1)
        ScreenGui:Destroy()
